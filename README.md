...
!["Primeira modelagem no Excel"](Tabela-Conceitual-Excel1.png)

#### Modelagem Conceitual no Excel - 1ª Normalização
!["Primeira modelagem no Excel - 1ª Normalização"](Tabela-Conceitual-Excel2.png)

#### Modelagem Conceitual no Excel - 2ª Normalização
!["Primeira modelagem no Excel - 2ª Normalização"](Tabela-Conceitual-Excel3.png)

### Modelagem de Dados com o Diagrama de Entidade Relacional
#### Neste diagrama temos as cardenalidades
    - Um para Muitos
    - Um para Um
!["Segundo modelagem"](Modelo-Conceitual-PaperBook.png)
...
!["Modelagem Lógica - Diagrama"](mer-PaperBook.png)
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Funcionario` (
  ...
  `rgfuncionario` VARCHAR(30) NOT NULL,
  ...
  INDEX `fk_funcionario_pk_sup_idx` (`superior` ASC) ,
  ...
  CONSTRAINT `fk_funcionario_pk_usuario`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_funcionario_pk_sup`
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Fornecedor` (
  ...
  `tipodeproduto` VARCHAR(50) NOT NULL,
  `idendereco` INT NOT NULL,
  `idcontato` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Categoria` (
  ...
  `descricaocategoria` VARCHAR(40) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Lote` (
  ...
  `numerolote` VARCHAR(40) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `paperbook`.`Pedido` (
  ...
  `unidadeloja` VARCHAR(30) NOT NULL,
  `numeroprotocolo` VARCHAR(60) NOT NULL,
  `formapagamento` VARCHAR(40) NOT NULL,
  `parcelas` VARCHAR(15) NULL,
  `valorparcelas` DECIMAL(5,2) NULL,
